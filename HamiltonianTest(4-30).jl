...
#I want to downsample the points of the pulse, so make 130 pts instead of 54132
function down_sample(p1, p2, t, new_pts)
    old_pts = size(p1)[2]
    t_downsampled_target = range(first(t), last(t), length = new_pts)

    # indices = round.(Int, range(1,old_pts, length = new_pts))
    indices = [findmin(abs.(t .- tt))[2] for tt in t_downsampled_target]
    println(indices)
    p1_downsampled = p1[:,indices]
    p2_downsampled = p2[:,indices]
    t_downsampled = t[indices]
    return p1_downsampled, p2_downsampled, t_downsampled
end
...
display(pt[1,1:5])
display(qt[1,1:5])
display(norm(p1[2,:] - qt[1,:]))
println(size(p1))
println(size(p2))
p1_down, p2_down, t_down = down_sample(p1, p2, t_list, 150)
# p = plot(t_list, p1')
# p2 = plot(t_list, p2')
# p_plots = plot(p, p2, layout = (2,1))
# display(p_plots)

# readline()

# p = plot(t_down, p1_down')
# p2 = plot(t_down, p2_down')
# p_plots = plot(p, p2, layout = (2,1))
# display(p_plots)

pt_correct_unit, qt_correct_unit, t_list = 
